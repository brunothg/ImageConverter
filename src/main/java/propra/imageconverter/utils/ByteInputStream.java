...
public class ByteInputStream extends InputStream implements Closeable {
	...
	return new String(readOrderedBytes(length), charset);
    }
	...
	final byte[] bytes = readOrderedBytes(2);
	if (bytes.length != 2) {
	...
	final byte[] bytes = readOrderedBytes(4);
	if (bytes.length != 4) {
	    throw new IOException("Nicht genügend bytes vorhanden");
	}
	...
	final byte[] bytes = readOrderedBytes(length);
